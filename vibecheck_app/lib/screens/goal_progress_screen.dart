...
class GoalProgressScreen extends StatelessWidget {
      ...
      body: SafeArea(
        child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            final int selectedDay = data['selectedDay'] as int? ?? 1;
            final int unlockedUntil = data['unlockedUntil'] as int? ?? 3;
            ...
            if (isCompleted) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(
                    builder: (_) => CongratulationScreen(goal: goal),
                  ),
                );
              });
            }

            final currentPlan = goal.monthlyPlans.firstWhere(
              (e) => e.day == selectedDay,
              orElse: () => goal.monthlyPlans.first,
            );

            final progress = goal.monthlyPlans.isEmpty
                ? 0.0
                : completedDays.length / goal.monthlyPlans.length;
                ...
                Positioned(
                  ...
                  child: Center(
                    child: AnimatedActionButton(
                      text: completedDays.contains(selectedDay)
                          ? 'Completed'
                          : 'Mark Day $selectedDay as Done',
                      icon: completedDays.contains(selectedDay)
                          ? Icons.check
                          : Icons.play_arrow,
                      onTap: completedDays.contains(selectedDay)
                          ? () {}
                          : () async {
                              await GoalProgressService.completeDay(
                                day: selectedDay,
                                totalDays: goal.monthlyPlans.length,
                              );
                            },
                    ),
                  ),
  ...
  List<Widget> _buildUpcomingGuideCards({
    required List<GoalDayPlan> plans,
    required int selectedDay,
  }) {
    final startIndex = selectedDay;
    ...
    return visiblePlans.map((plan) {
      return Padding(
        ...
        child: _GuideCard(
          stripColor: const Color(0xFF8E6CE3),
          title: plan.title,
